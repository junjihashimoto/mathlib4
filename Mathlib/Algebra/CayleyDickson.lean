...
ones with zero divisors, such as the sedenions), which is what the hypercomplex Fourier
transform needs; see `Mathlib/Analysis/Fourier/CayleyDicksonPlancherel.lean`.
